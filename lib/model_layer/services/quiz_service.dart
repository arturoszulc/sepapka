...
class QuizService {
  ...
  QuizService(this._ref) {
    log.log('^^^ QuizService initialized ^^^');
  }
  ...
  //Get quizQuestionList with cut-out hidden questions
  List<Question> getQuizQuestionList() {
    final List<Question> quizList = List.from(questionListGlobalConst);
    quizList.removeWhere((e) =>
        _ref
            .read(userService)
            .hiddenQuestionIds
            .contains(e.id));
  ...
  //   final List<Question> quizList = List.from(questionListGlobalConst);
  //   quizList.removeWhere((q) => q.level != qLevel); //filter by level
  //   quizList.removeWhere((q) => q.label != qCategory); //filter by category
  //   quizList.removeWhere((e) => _userService.isQuestionInNotShownList(e.id) != null); //remove if on NotShownList
  //   quizList.shuffle(); //shuffle the list
  //   return quizList;
  // }
